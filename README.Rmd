...
```{r include=FALSE}
# We need a workaround to include htmlwidgets in github_document output format.
...
iframe_htmlwidget <- function(htmlwidget) {
  ...
  #       it seems it is, but can't seems to find assurances
...
iframe_htmlwidget(wdiquickplots::plot_bar_wdi_ind("NY.GDP.PCAP.PP.CD"))
```
...
```{r race, out.width="40%", fig.asp=550/350, fig.width=5, cache=TRUE}
...
iframe_htmlwidget(wdiquickplots::plot_time_wdi_ind("NY.GDP.PCAP.PP.CD"))
```
...
iframe_htmlwidget(wdiquickplots::plot_spaghetti_wdi_ind("NY.GDP.PCAP.PP.CD"))
```
